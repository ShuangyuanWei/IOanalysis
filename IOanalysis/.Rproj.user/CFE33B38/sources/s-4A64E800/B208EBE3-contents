...
process_io_tables <- function(dirname, countryname) {
  ...
  isic_earn <- oecd_emp_earn %>% group_by(classif1.label) %>% summarize(total_earn = sum(value, na.rm = TRUE))
  ...
  write.csv( combined_m , file =paste0("type2_multipliers",".csv"))
}
